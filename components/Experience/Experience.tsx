...
// Smart matching function that looks for keywords instead of exact sentences
const getCompanyInitials = (company: string): string => {
  if (!company) return '??';
  
  const cleanName = company.toLowerCase();

  if (cleanName.includes('seminole') || cleanName.includes('hardrock') || cleanName.includes('hard rock')) {
    return 'SHR';
  }
  if (cleanName.includes('children') || cleanName.includes('museum')) {
    return 'TCM';
  }
  if (cleanName.includes('indiana')) {
    return 'ISU';
  }
  if (cleanName.includes('srm')) {
    return 'SRM';
  }
            ...
            >
              <div className="xp__content-wrapper">
                ...
                >
                  ...
                  {/* FIX: This safely handles both string descriptions and multi-line array descriptions */}
                  {item.description && (
                    <div className="xp__desc-container" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      {Array.isArray(item.description) ? (
                        item.description.map((paragraph, index) => (
                          <p className="xp__desc" key={index} style={{ marginBottom: '4px' }}>
                            {paragraph}
                          </p>
                        ))
                      ) : (
